...
async def list_clusters(response: Response,category: Optional[str] = None, tunisian: bool = False,sort: Literal["latest","top"]= Query(default="top"),
    ...
    if tunisian:
        return await get_clusters_about_tunisia(db,sort,window,as_of,limit,offset)
    if category:
        return await get_clusters_by_category(db, category,sort,window,as_of,limit,offset)
    return await get_clusters(db,sort,window,as_of,limit,offset)
